...
switch ( eyes_state ) 
{
  ...
  case BLUSHHOLD:
    ...
    if (current_button == 1) {eyes_state = BLUSHEND;}
    }
   break;
  case BLUSHSTART:
    if (CompareTime(current_timer, lastactive_timer) >= current_delay){
    ...
    if (current_delay==0){
      if (current_button == 0)  {eyes_state = BLUSHHOLD; Serial.println(F("going to hold"));}
      if (current_button== 1) {eyes_state = BLUSHEND; Serial.println(F("going to end"));}
    }
    break;
}
...
if(current_delay == 0 && current_button == 0 && blushbutton_waspushed == false){
// Pushed
